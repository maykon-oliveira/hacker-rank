...
fun countApplesAndOranges(
    ...
    val applesDistances = apples.map { it + appleTree }
    val oragesDistances = oranges.map { it + orangeTree }

    val appleCount = applesDistances.count {
        it in s..t
    }

    println(appleCount)

    val orangeCount = oragesDistances.count {
        it in s..t
    }
